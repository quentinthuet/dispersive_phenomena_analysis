...
xmin = 0.0; xmax = 1.0; nptx = 31; nx = nptx-2  
a = xmax-xmin
hx = a/(nptx -1)
...
def creer_probleme_neumann(nptx,npty,nx,ny,hx,hy,a,b,n=1,k=1):
    # Source term
    def Source_int(x):
        return np.pi**2*np.sin(np.pi*x[1]/b)*(np.cos(np.pi*x[0]/a)/b**2+np.cos(np.pi*x[0]/a)/a**2-1/a**2)
    def Source_bnd(x):
        return 0
    def Sol_sin(x):
...
def creer_probleme_neumann_v2(nptx,npty,nx,ny,hx,hy,a,b,vx,n=1,k=1):
    # Source term
    def Source_int(x):
        return np.pi**2*np.sin(np.pi*x[1]/b)*(np.cos(np.pi*x[0]/a)/b**2+np.cos(np.pi*x[0]/a)/a**2-1/a**2)+vx*(-np.pi/a)*np.sin(np.pi*x[1]/b)*np.sin(np.pi*x[0]/a)
    def Source_bnd(x):
        return 0
    def Sol_sin(x):
        return (np.cos(np.pi*x[0]/a)-1)*np.sin(np.pi*x[1]/b)    
...
u_ex_neumann, F_neumann = creer_probleme_neumann(nptx,npty,nx,ny,hx,hy,a,b,vx)

u_neumann = npl.solve(A2,F_neumann)

#Post-traintement u_ex+Visualization of the exct solution
uu_ex = np.reshape(u_ex_neumann,(nx+2 ,ny+2),order = 'F');
uu = np.reshape(u_neumann,(nx+2 ,ny+2),order = 'F');
X,Y = np.meshgrid(xx,yy)
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(X,Y,uu_ex.T,rstride = 1, cstride = 1);
...
print('Norme de l\'erreur (Neumann) : ',np.linalg.norm(uu_ex-uu))



u_ex_neumann_v2, F_neumann_v2 = creer_probleme_neumann_v2(nptx,npty,nx,ny,hx,hy,a,b,vx)

u_neumann_v2 = npl.solve(A2,F_neumann_v2)

#Post-traintement u_ex+Visualization of the exct solution
uu_ex = np.reshape(u_ex_neumann_v2,(nx+2 ,ny+2),order = 'F');
uu = np.reshape(u_neumann_v2,(nx+2 ,ny+2),order = 'F');
X,Y = np.meshgrid(xx,yy)
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(X,Y,uu_ex.T,rstride = 1, cstride = 1);
...
print('Norme de l\'erreur (Neumann avec terme advectif) : ',np.linalg.norm(uu_ex-uu))
